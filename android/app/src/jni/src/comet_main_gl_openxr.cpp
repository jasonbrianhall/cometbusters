...
static bool init_sdl_and_opengl(CometGUI *gui, int width, int height) {
    ...
    printf("[XR] Attempting to initialize OpenXR...\n");
    if (openxr_init(&gui->xr_ctx) == 0) {
        fprintf(stderr, "[XR] OpenXR initialized successfully!\n");
        gui->use_xr = true;
    } else {
        fprintf(stderr, "[XR] OpenXR initialization failed, falling back to desktop rendering\n");
        gui->use_xr = false;
        memset(&gui->xr_ctx, 0, sizeof(OpenXRContext));
    }
...
static void render_frame(CometGUI *gui, HighScoreEntryUI *hs_entry, CheatMenuUI *cheat_menu) {
    // IF USING VR - RENDER STEREO
    if (gui->use_xr) {
        int should_render = openxr_begin_frame(&gui->xr_ctx);
        if (should_render <= 0) {
            if (should_render == 0) openxr_end_frame(&gui->xr_ctx);
            return;
        }
        ...
        for (int eye = 0; eye < 2; eye++) {
            ...
            openxr_get_eye_matrices(&gui->xr_ctx, eye, proj, view, &width, &height);
            ...
            xrAcquireSwapchainImage(gui->xr_ctx.swapchain, &acquire_info, &swapchain_index);
            
            // Bind eye framebuffer
            glBindFramebuffer(GL_FRAMEBUFFER, gui->xr_ctx.swapchain_fbos[swapchain_index]);
            ...
            glLoadIdentity();
            glOrtho(0, 1920.0f, 1080.0f, 0, -1, 1);
            ...
            xrReleaseSwapchainImage(gui->xr_ctx.swapchain, &release_info);
        }
...
static void cleanup(CometGUI *gui) {
    // Cleanup OpenXR if it was used
    if (gui->use_xr) {
        openxr_shutdown(&gui->xr_ctx);
    }
...
int main(int argc __attribute__((unused)), char *argv[] __attribute__((unused))) {
    ...
    gui.use_xr = false;  // Initialize VR flag
    ...
    if (!gui.use_xr) {
        // Window is now maximized, get the actual size (only for desktop)
        SDL_GetWindowSize(gui.window, &gui.window_width, &gui.window_height);
        printf("[INIT] Window maximized size: %dx%d\n", gui.window_width, gui.window_height);
    } else {
        printf("[XR] Running in VR mode\n");
    ...
    while (gui.running) {
        ...
        // Get timing from OpenXR if available, otherwise use SDL
        if (gui.use_xr) {
            gui.delta_time = gui.xr_ctx.frame_time;
        } else {
        ...
        // Frame rate limiting (only for desktop mode)
        if (!gui.use_xr) {
