...
CXXFLAGS_LINUX = $(CXXFLAGS_COMMON) $(SDL2_CFLAGS_LINUX) $(GTK_CFLAGS_LINUX) $(SDL2_MIXER_CFLAGS_LINUX) -DExternalSound -DCAIROBUILD -DLINUX -DVERSION=\"$(VERSION)\"
...
cometbuster-create-wad:
	@echo "Building WAD"
	python createwad.py
	for mydir in build/linux build/windows build/linux_debug build/windows_debug; do cp cometbuster.wad $$mydir/; done
