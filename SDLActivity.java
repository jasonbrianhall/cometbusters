...
*/
    ...
    /**
     * Extract WAD file from APK assets if it doesn't already exist
     */
    private void ensureWADExtracted() {
        try {
            String wadFileName = "cometbuster.wad";
            java.io.File wadFile = new java.io.File(getFilesDir(), wadFileName);
            
            // Check if file already exists
            if (wadFile.exists() && wadFile.length() > 0) {
                Log.v(TAG, "WAD file already exists: " + wadFile.getAbsolutePath() + 
                      " (" + wadFile.length() + " bytes)");
                return;
            }
            
            // Extract from assets
            Log.v(TAG, "Extracting WAD file from assets: " + wadFileName);
            ...
            Log.v(TAG, "WAD extraction complete: " + wadFile.getAbsolutePath() + 
                  " (" + totalBytes + " bytes)");
            
        } catch (java.io.FileNotFoundException e) {
            Log.w(TAG, "WAD file not found in assets: " + e.getMessage());
        } catch (Exception e) {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Hide status bar and window decorations
    ...
    {
        // Force landscape orientation only - ignore all hints
        Log.v(TAG, "setOrientation() forced to LANDSCAPE - width=" + w +" height="+ h +" resizable=" + resizable + " hint=" + hint);
