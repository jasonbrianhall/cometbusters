...
void comet_buster_spawn_ship_death_explosion(CometBusterGame *game, double x, double y) {
    ...
    SDL_Log("[Explosion] Ship destroyed at (%.0f, %.0f) - dealt damage in %.0f pixel radius\n", 
...
void comet_buster_spawn_boss(CometBusterGame *game, int screen_width, int screen_height) {
    ...
    SDL_Log("[SPAWN BOSS] Attempting to spawn boss at Wave %d\n", game->current_wave);
    ...
    SDL_Log("[SPAWN BOSS] Boss spawned! Position: (%.1f, %.1f), Active: %d, Health: %d\n", 
...
/*void comet_buster_spawn_star_boss(CometBusterGame *game, int screen_width, int screen_height) {
    ...
    SDL_Log("[SPAWN STAR BOSS] Rotating star boss spawned! Position: (%.1f, %.1f)\n", 
            boss->x, boss->y);
}*/
