...
/**
 * Map joystick input to keyboard inputs
 * Uses OR logic: if either keyboard OR joystick says key is pressed, it's pressed
 * Keyboard events handle setting keys to false on key release
 */
static void map_joystick_to_keyboard(Visualizer *vis) {
    if (!vis || !vis->joystick_manager.joystick_enabled) return;
    
    JoystickState *joy = joystick_manager_get_active(&vis->joystick_manager);
    if (!joy || !joy->connected) return;
    
    // Threshold: 0.3 to avoid noise from analog drift
    const double STICK_THRESHOLD = 0.3;
    bool joystick_active = false;
    
    // Left stick X-axis: left/right movement
    if (joy->axis_x < -STICK_THRESHOLD) {
        vis->key_a_pressed = true;  // Left
        joystick_active = true;
    }
    if (joy->axis_x > STICK_THRESHOLD) {
        vis->key_d_pressed = true;  // Right
        joystick_active = true;
    }
    
    // Left stick Y-axis: forward/backward
    if (joy->axis_y < -STICK_THRESHOLD) {
        vis->key_w_pressed = true;  // Forward/Up
        joystick_active = true;
    }
    if (joy->axis_y > STICK_THRESHOLD) {
        vis->key_s_pressed = true;  // Backward/Down
        joystick_active = true;
    }
    
    // Buttons mapped to game actions
    if (joy->button_a) {
        vis->key_z_pressed = true;  // A = Z (omnidirectional fire)
        joystick_active = true;
    }
    
    if (joy->button_b) {
        vis->key_x_pressed = true;  // B = X (boost/secondary fire)
        joystick_active = true;
    }
    
    if (joy->button_x) {
        vis->key_space_pressed = true;  // X = Space (boost)
        joystick_active = true;
    }
    
    if (joy->button_y) {
        vis->key_q_pressed = true;  // Y = Q (weapon toggle)
        joystick_active = true;
    }
    
    // D-Pad for movement as well
    if (joy->dpad_left) {
        vis->key_a_pressed = true;
        joystick_active = true;
    }
    if (joy->dpad_right) {
        vis->key_d_pressed = true;
        joystick_active = true;
    }
    if (joy->dpad_up) {
        vis->key_w_pressed = true;
        joystick_active = true;
    }
    if (joy->dpad_down) {
        vis->key_s_pressed = true;
        joystick_active = true;
    }
    
    // If any joystick input is active, disable mouse aiming (like keyboard does)
    if (joystick_active) {
        vis->mouse_just_moved = false;
    }
}
